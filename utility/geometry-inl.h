...
template<class T>
T distance(const Plane3<T> & plane, const Vector3<T> & point)
{
    T t = intersectionTime(plane, point, unit(plane.normal()));
...
template<typename T>
static Triangle2<T> sProject(const Triangle3<T> & tri, int u0, int u1)
{
    return Triangle2<T>(Vector2<T>(tri[0][u0], tri[0][u1]),
        Vector2<T>(tri[1][u0], tri[1][u1]),
        Vector2<T>(tri[2][u0], tri[2][u1]));
}

template<typename T>
bool coplanar(const Triangle3<T> & t1, const Triangle3<T> & t2, const T & relTol)
{
    T radiansThreshold = 1e-4;
    Vector3<T> n1 = t1.normal();
    Vector3<T> n2 = t2.normal();
    
    if (!parallel(n1, n2, radiansThreshold))
    {
        return false;
    }
    
    // 2. Triangles are roughly parallel.  Now do a point-plane distance test.
    T distanceThreshold = 1e-10; // TODO: pick this intelligently
    Plane3<T> plane1 = t1.plane();
    T distToPlane = distance(plane1, t2[0]);

    if (distToPlane > distanceThreshold)
    {
        return false;
    }
    return true;
...
template<typename T>
bool coincident(const Triangle3<T> & t1, const Triangle3<T> & t2, const T & relTol)
{
    // 1. Find sin(theta)^2 between the normal vectors.
    // Compare to an angle in radians to determine if the normals are parallel.
    T radiansThreshold = 1e-4;
    T sin2threshold = radiansThreshold*radiansThreshold;
    
    Vector3<T> n1 = t1.normal();
    Vector3<T> n2 = t2.normal();
    T dotProd = dot(n1, n2);
    T sin2theta = 1.0 - dotProd*dotProd/(sumSquares(n1)*sumSquares(n2));
    
    if (sin2theta > sin2threshold)
    {
        return false;
    }
    ...
    // 3. Triangles lie on the same plane.  Now test whether they intersect.
    // - project to 2D
    // - test if any edges intersect (return true)
    // - do a single point-in-triangle test (return true)
    // - else return false
    
    int u2 = dominantDirection(n1);
    int u0 = (u2+1)%3;
    int u1 = (u2+2)%3;
    
    Triangle2<T> t1_2d = sProject(t1, u0, u1);
    Triangle2<T> t2_2d = sProject(t2, u0, u1);
    
    if (intersects(t1_2d, t2_2d, relTol))
    {
        return true;
    }
    else
