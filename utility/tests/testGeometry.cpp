...
BOOST_AUTO_TEST_CASE( TestSegmentIntersection )
{
    Vector2d p0(0,0), p1(1,0);
    Vector2d q0(0,-1), q1(1,1);
    Vector2d r0(0,1e-4), r1(1, 1e-4);
    Vector2d s0(2, -1), s1(1, 1);
    
    BOOST_CHECK(segmentIntersects(p0, p1, q0, q1, 0.0));
    BOOST_CHECK(segmentIntersects(r0, r1, q0, q1, 0.0));
    BOOST_CHECK(segmentIntersects(p0, p1, s0, s1, 0.0) == false);
    BOOST_CHECK(segmentIntersects(p0, p1, r0, r1, 0.0) == false);
}
...
BOOST_AUTO_TEST_CASE( TestTriangleIntersection )
{
    Triangle2d t1(Vector2d(0,0), Vector2d(1,0), Vector2d(0,1));
    Triangle2d t2(Vector2d(0,-0.5), Vector2d(1,-0.5), Vector2d(0,0.5));
    Triangle2d t3(Vector2d(0,-1.5), Vector2d(1,-1.5), Vector2d(0,-0.5));
    
    BOOST_CHECK(intersects(t1, t2, 0.0));
    BOOST_CHECK(intersects(t1, t3, 0.0) == false);
}

BOOST_AUTO_TEST_CASE( TestTriangleCoincidence )
{
    // Coincident = coplanar + intersecting.
    Triangle3d t1(Vector3d(0,0,0), Vector3d(1,0,0), Vector3d(0,1,0));
    Triangle3d t2(Vector3d(0,0,-0.1), Vector3d(1,0,0.1), Vector3d(0,1,0.1));
    Triangle3d t3(Vector3d(0,0,-1e-6), Vector3d(1,0,-1e-6), Vector3d(0,1,-1e-6));
    Triangle3d t4(Vector3d(0,0,0), Vector3d(0.5,-1,0), Vector3d(1,1,0));
    Triangle3d t5(Vector3d(0.1,0.1,0), Vector3d(0.9,0.1,0), Vector3d(0.1,0.9,0));
    Triangle3d t6(Vector3d(1.1,0.1,0), Vector3d(1.9,0.1,0), Vector3d(1.1,0.9,0));
    
    BOOST_CHECK(coincident(t1, t2, 0.0) == false); // intersecting but not coplanar
    BOOST_CHECK(coincident(t1, t3, 0.0) == false); // parallel and overlapping in the XY plane but not coplanar
    BOOST_CHECK(coincident(t1, t4, 0.0) == true); // coplanar with intersecting edges
    BOOST_CHECK(coincident(t1, t5, 0.0) == true); // coplanar without intersecting edges
    BOOST_CHECK(coincident(t1, t6, 0.0) == false); // coplanar and non-intersecting
}
