...
std::ostream & operator<<(std::ostream & str, const RunlineSumEH_i & rl)
{
    str << "[ehi " << rl.ehi << " ehii " << rl.ehii
        << " ehij " << rl.ehij << " ehik " << rl.ehik << " length " << rl.length()
        << "]";
    return str;
}
...
{
    str << "[eh " << rl.eh << " db " << rl.db0 << "," << rl.db1 << "," << rl.db2
        << "," << rl.db3 << " aux " << rl.aux << " coeff " << rl.coeff
...
SumEH_i(Field ehi, Field ehii, Field ehij, Field ehik,
    const vector<RunlineSumEH_i> & runlines) :
...
unsigned long SumEH_i::
bytes() const
{
    return mRunlines.size() * sizeof(RunlineSumEH_i);
}
...
apply(long timestep, Precision::Float dt)
{
    for (int rr = 0; rr < mRunlines.size(); rr++)
    {
        ...
        Precision::Float* ehij = mHeadEHij + mRunlines[rr].ehij;
        Precision::Float* ehik = mHeadEHik + mRunlines[rr].ehik;
        
        for (int ll = 0; ll < mRunlines[rr].length(); ll++)
            ehi[ll] = ehii[ll] + ehij[ll] + ehik[ll];
    }
}
...
unsigned long UpdateAnisotropicEH_i::
bytes() const
{
    long totalBytes = 0;
    totalBytes += mRunlines.size()*sizeof(RunlineAnisotropicEH_i);
...
apply(long timestep, Precision::Float dt)
{
    for (int rr = 0; rr < mRunlines.size(); rr++)
    {
        ...
        Precision::Float* db2 = mHeadDB + mRunlines[rr].db2;
        Precision::Float* db3 = mHeadDB + mRunlines[rr].db3;
...
//            Precision::Float* constEH = &(mCoefficientsEH[0].at(mRunlines[rr].coeff));
        ...
        for (int ll = 0; ll < mRunlines[rr].length(); ll++)
        {
            ...
            Precision::Float meanDB = 0.25*(*db0 + *db1 + *db2 + *db3);
